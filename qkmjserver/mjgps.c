...
list_stat (int fd, char *name)
{
    ...
    sprintf (msg_buf1, "101◇金額:%ld 排名:%s 上線次數:%d 已玩局數:%d",
     ...
     char *name;
{
	...
	if (player[id].login == 2)
	{
	    ...
	    if (player[id].join)
	    {
		sprintf (msg_buf, "101◇%s 在 %s 桌內", name, player[player[id].join].name);
		write_msg (fd, msg_buf);
	    }
	    if (player[id].serv)
	    {
...
check_user (int player_id)
{
	    ...
	    strcmp (player[player_id].username, msg_buf) == 0)
	{
	    display_msg (player_id,
		  "你已被限制進入, 有問題請 mail 給 sywu@csie.nctu.edu.tw");
	    fclose (baduser_fp);
	    return 0;
	}
    }
...
welcome_user (int player_id)
{
    ...
    sprintf (msg_buf, "101★★★★★　歡迎 %s 來到ＱＫ麻將  ★★★★★",
    ...
    sprintf (msg_buf, "101◇目前上線人數: %d 人       注冊人數: %d 人", online_num,
	     total_num);
    write_msg (player[player_id].sockfd, msg_buf);
 ...
 */
		...
		{
		    write_msg (player[player_id].sockfd, "101對不起,目前使用人數超過上限, 請稍後再進來.");
